...
parser = ap.ArgumentParser(description="Computes the Power Spectrum P(k) of a simulated catalog of sources and measures the bias parameters up to 1-loop. For large datasets it's strongly advised to run this script using openMPI (add mpirun before issuing python).\n"
                                       ...
                                       "\033[7m*** USAGE ***\033[0m\n"
                                       "To compute auto P(k):\n"
                                       "    python3 pipeline.py /path/to/init_file autopk\n"
                                       "\n"
                                       "To compute cross P(k):\n"
                                       "    python3 pipeline.py /path/to/init_file crosspk /path/to/pk_file1 /path/to/pk_file2\n"
                                       "\n"
                                       "To compute P(k) and bias:\n"
                                       "    python3 pipeline.py /path/to/init_file (autopk/crosspk [args])  --do-bias\n"
                                       "\n"
                                       "To compute bias from P(k) file:\n"
                                       "    python3 pipeline.py /path/to/init_file load /path/to/pk_file\n"
                                       "*************\n",
...
if "mpirun" in os.environ["_"] or any("MPI" in i for i in os.environ.keys()):
    functions.printflush("Running program through MPI parallelization")
    use_mpi = True

else:
    functions.printflush("Running program with multiprocessing package")
    use_mpi = False
...
if args.command=="autopk":
    ...
    shutil.copy(init["Pk_parameters"]["read_sim_func"]["file_path"], "./tmp.py")
    ...
    os.chmod(out_path, mode=0o775)
    os.chown(out_path, uid=uid, gid=gid)
    ...
    functions.printflush("Files saved in {}".format(out_path))
    ...
    pk1 = pb.PowerSpectrum()
    pk1.load(pk_file1)
    pk2 = pb.PowerSpectrum()
    pk2.load(pk_file2)
    
    crosspk = pb.PowerSpectrum(**pk_params)

...
misc.printflush("starting MAS computation on {} particles (indexes {} to {})".format(len(local_pos), start, stop), rank)

for pos, count in zip(local_pos, counts):
    weights, indexes = misc.assignDoubleGrid(pos, L, Ng)
    for i in range(len(weights)):
        local_rho[indexes[i]] += count*weights[i]/H**3

misc.printflush("has completed MAS computation, sending MAS results to root".format(rank), rank)
